...
"""
Perform the actual rendering of the MIDI files on the SLURM cluster.

Usage:
    uv run submit_slurm_workers.py -c <maestro_csv_path> -o <output_dir> -b <blender_path>

IMPORTANT Note 1:
- The Maestro CSV file should be in the format of the Maestro dataset, PLUS a 'worker_id' column!
- The 'worker_id' column should be an integer which assigns every MIDI file to one worker (Slurm job)which will render it.
- i.e. this script will submit 1 Slurm job for each worker_id.
- See the data/maestro-v3.0.0/maestro-v3.0.0.csv files in this repo as an example.

IMPORTANT Note 2:
- The `midi_filename` column from the CSV should correspond to the file paths based on `data/maestro-v3.0.0/`.
- If you just drop the unzipped Maestro dataset into the `data/maestro-v3.0.0/` folder, it should work! Make sure to adapt the CSV file according to Note 1.
...
Example:
    uv run submit_slurm_workers.py \
    -c /home/stud/wfel/repos/piano-videos-dataset/data/maestro-v3.0.0/maestro-v3.0.0.csv \
    -o /storage/remote/atcremers125/wfel/maestro-visualized \
    -b /home/stud/wfel/blender-4.4.3-linux-x64/blender
"""
